...
<div class="container mt-4">
    <div class="row">
        ...
        <form id="filterForm" class="mb-3 d-inline-block">
            <input type="date" name="start_date" value="{{ today()->toDateString() }}" class="form-control d-inline w-auto">
            <button type="submit" class="btn btn-primary">Load Week</button>
        </form>
        <button type="button" id="cloneWeekBtn" class="btn btn-success ms-2">Clone Week → Next 7 Days</button>
...
<script>
    ...
    function buildGrid(gridData, prefix, roleMap, columns, kitchenOnlyDay = false) {
        ...
        const headHtml = `<tr><th>Action</th>${columns.map(c => `<th>${c}</th>`).join('')}</tr>`;
        ...
        gridData.forEach(row => {
            ...
            if (!kitchenOnlyDay) {
                ...
                nightRows.splice(idxN + 1, 0, `<tr><td>${row.name}</td>${nightCells.join('')}</tr>`);
            }
        });

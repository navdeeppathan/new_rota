...
<div class="container mt-4">
    <div class="row">
        ...
        <form id="filterForm" class="mb-3">
            <select name="year" id="yearSelect" class="form-select d-inline w-auto">
        ...
        <div class="mt-3 mb-3 p-2 col-md-5" style="padding: 10px; margin-left: 11px; border: 2px solid;">
            ...
            <form id="cloneWeekForm" class="row g-2">
                <div class="col-auto">
                    <select id="cloneYear" class="form-select" required>
                        <option value="">Select Year</option>
                        @for ($y = 2024; $y <= 2030; $y++)
                            <option value="{{ $y }}">{{ $y }}</option>
                        @endfor
...
<script>
    ...
    //         });
    // });
    document.getElementById('cloneYear').addEventListener('change', function () {
    const year = this.value;
    if (!year) return;

    fetch(`/api/weeks?year=${year}`)
        .then(r => r.json())
        .then(res => {
            if (!res.status) return;
            ...
            Object.entries(res.weeks).forEach(([month, weeks]) => {
                ...
                og1.label = `${month} ${year}`;

                const og2 = document.createElement('optgroup');
                og2.label = `${month} ${year}`;

                weeks.forEach(week => {
                    og1.append(new Option(week.label, week.value));
                    og2.append(new Option(week.label, week.value));
                });

                sourceWeek.appendChild(og1);
                targetWeek.appendChild(og2);
            });
        });
});
    ...
    document.getElementById('cloneWeekForm').addEventListener('submit', function(e) {
        ...
        const year = document.getElementById('cloneYear').value;
